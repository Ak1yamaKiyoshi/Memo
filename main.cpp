...
int main(int argc, char* argv[]) {
    ...
    std::vector<std::string> vocaburary;
    ...
    if (argc > 1) {
        ...
        if (!command.compare("/add") || !command.compare("/a")) {
            ...
            if (trim(arguments).length() == 0) {
                message(error_user_side, "You  no provided search query.");
                return -1;
            }

            std::string searchQuery = arguments;
            time_t search_start = time(0);
            std::vector<search_result> rs = search(searchQuery, memories, vocaburary);
            ...
            for (auto result : rs) {
                ...
                std::cout << "[" << ANSI_BRIGHT_WHITE << std::fixed << std::setprecision(2)
                          << std::setw(5) << std::setfill(' ') << result.confidence << " ] " << ANSI_RESET
                          << memo_to_stringc(result.reference) << std::endl;
            }
        } else {
        ...
        while (is_running) {
            ...
            if (!command.compare("/add") || !command.compare("/a")) {
                ...
                time_t search_start = time(0);
                std::vector<search_result> rs = search(arguments, memories, vocaburary);
                time_t search_end = time(0);
                time_t diff = search_end - search_start;
                float time_taken = static_cast<float>(diff);
                ...
                help_message 
                    ...
                    << "Note:\n"
                    << " - In case of passing command to binary, tags should be specified not with '#' but '@'. \n"
                    << " ./memo /search @tag text \n";
